...
if (Isfitool === undefined) var Isfitool = {};

Isfitool.defaultComponent = "_base";
Isfitool.preservePropertySections = true;
Isfitool.dragIcon = 'icon';//icon = use component icon when dragging | html = use component html to create draggable element

Isfitool.baseUrl =  document.currentScript?document.currentScript.src.replace(/[^\/]*?\.js$/,''):'';

Isfitool.ComponentsGroup = {};
Isfitool.BlocksGroup = {};
...
*/
	...
	render: function(type) {
		...
		if (!(Isfitool.preservePropertySections && section.length))
		{
			componentsPanelSections[defaultSection].html('').append(tmpl("isfitool-input-sectioninput", {key:"default", header:component.name}));
		...
		if (component.beforeInit) component.beforeInit(Isfitool.Builder.selectedEl.get(0));
		...
		var fn = function(component, property) {
			return property.input.on('propertyChange', function (event, value, input) {
					...
					var element = Isfitool.Builder.selectedEl;
					...
					if (property.htmlAttr)
					{
						...
						{
							element = Isfitool.StyleManager.setStyle(element, property.key, value);
						}
						else if (property.htmlAttr == "innerHTML") 
						{
							element = Isfitool.ContentManager.setHtml(element, value);
						}
						else
						{
						...
						Isfitool.Undo.addMutation({type: 'attributes', 
					...
					if (!property.child && !property.parent) Isfitool.Builder.selectNode(element);
					
					return element;
			});				
		};			
	
		var nodeElement = Isfitool.Builder.selectedEl;
		...
		for (var i in component.properties)
		{
			...
			} else if (property.htmlAttr)
			{
				if (property.htmlAttr == "style")
				{
					...
					var value = Isfitool.StyleManager.getStyle(element, property.key);//getStyle returns declared style
				} else
				if (property.htmlAttr == "innerHTML")
				{
					var value = Isfitool.ContentManager.getHtml(element);
				} else
				{
			...
			{
				var row = $(tmpl('isfitool-property', property)); 
		...
		if (component.init) component.init(Isfitool.Builder.selectedEl.get(0));
	}
};	
...
Isfitool.WysiwygEditor = {
	...
	destroy: function(element) {
		...
		Isfitool.Undo.addMutation({type:'characterData', 
								target: node, 
...
/* controls */    	
	loadControlGroups : function() {	
		...
		componentsList.each(function ()
		{
			...
			for (group in Isfitool.ComponentsGroup)	
			{
				...
				components = Isfitool.ComponentsGroup[ group ];
				
				for (i in components)
				{
					componentType = components[i];
					component = Isfitool.Components.get(componentType);
	...
	loadBlockGroups : function() {	
		...
		blocksList.each(function ()
		{
			...
			for (group in Isfitool.BlocksGroup)	
			{
				...
				blocks = Isfitool.BlocksGroup[ group ];

				for (i in blocks)
				{
					blockType = blocks[i];
					block = Isfitool.Blocks.get(blockType);
	...
	loadUrl : function(url, callback) {	
		...
		self.initCallback = callback;
		if (Isfitool.Builder.iframe.src != url) Isfitool.Builder.iframe.src = url;
	},
...
/* iframe */
	_loadIframe : function(url) {	
	    ...
	    return this.documentFrame.on("load", function() 
        {
				...
				Isfitool.WysiwygEditor.init(window.FrameDocument);
    ...
    _frameLoaded : function() {
		...
		self.frameHead.append('<link data-isfitool-helpers href="' + Isfitool.baseUrl + '../../css/isfitooljs-editor-helpers.css" rel="stylesheet">');

		self._initHighlight();
		
		$(window).triggerHandler("isfitool.iframe.loaded", self.frameDoc);
    },	
	...
	loadNodeComponent:  function(node) {
		data = Isfitool.Components.matchNode(node);
		var component;
		
		if (data) 
			component = data.type;
		else 
			component = Isfitool.defaultComponent;
			
		Isfitool.Components.render(component);
	...
	selectNode:  function(node) {
		...
		{
			Isfitool.WysiwygEditor.destroy(self.texteditEl);
...
/* iframe highlight */    
    _initHighlight: function() {
		...
		self.frameHtml.on("mouseup touchend", function(event) {
			if (self.isDragging)
			{
				...
				if (self.dragMoveMutation === false)
				{
					Isfitool.Undo.addMutation({type: 'childList', 
				...
				} else
				{
					...
					Isfitool.Undo.addMutation(self.dragMoveMutation);
		...
		self.frameHtml.on("dblclick", function(event) {
			...
			if (Isfitool.Builder.isPreview == false)
			{
				self.texteditEl = target = jQuery(event.target);

				Isfitool.WysiwygEditor.edit(self.texteditEl);
	...
	_initBox: function() {
		...
		$("#down-btn").on("click", function(event) {
			...
			Isfitool.Undo.addMutation({type: 'move', 
									target: node,
									oldParent: oldParent,
									newParent: newParent,
		...
		$(".components-list li ol li", addSectionBox).on("click", function(event) {
			var html = Isfitool.Components.get(this.dataset.type).html;
		...
		$(".blocks-list li ol li", addSectionBox).on("click", function(event) {
			var html = Isfitool.Blocks.get(this.dataset.type).html;
...
/* drag and drop */
	_initDragdrop : function() {
		...
		$('.drag-elements-sidepane ul > li > ol > li').on("mousedown touchstart", function(event) {
			...
			if ($this.data("drag-type") == "component")
				self.component = Isfitool.Components.get($this.data("type"));
			else
				self.component = Isfitool.Blocks.get($this.data("type"));
	...
	removeHelpers: function (html, keepHelperAttributes = false)
	{
		...
		html = html.replace(/<.*?data-isfitool-helpers.*?>/gi, "");
		//attributes
		if (!keepHelperAttributes)
		{
			html = html.replace(/\s*data-isfitool-\w+(=["'].*?["'])?\s*/gi, "");
		}
	...
	getHtml: function(keepHelperAttributes = true) 
	{
		...
		$(window).triggerHandler("isfitool.getHtml.before", doc);
		...
		"<!DOCTYPE "
         ...
         var filter = $(window).triggerHandler("isfitool.getHtml.after", html);
	...
	{
		var data = {};
		data["fileName"] = (fileName && fileName != "") ? fileName : Isfitool.FileManager.getCurrentUrl();
...
Isfitool.CodeEditor = {
	...
	init: function(doc) {
		$("#isfitool-code-editor textarea").val(Isfitool.Builder.getHtml());

		$("#isfitool-code-editor textarea").keyup(function () 
		{
			delay(Isfitool.Builder.setHtml(this.value), 1000);
		});

		//load code on document changes
		Isfitool.Builder.frameBody.on("isfitool.undo.add isfitool.undo.restore", function (e) { Isfitool.CodeEditor.setValue();});
...
Isfitool.Gui = {
	...
	init: function() {
		$("[data-isfitool-action]").each(function () {
			on = "click";
			if (this.dataset.isfitoolOn) on = this.dataset.isfitoolOn;
			
			$(this).on(on, Isfitool.Gui[this.dataset.isfitoolAction]);
			if (this.dataset.vvvebShortcut)
			{
				$(document).bind('keydown', this.dataset.isfitoolShortcut, Isfitool.Gui[this.dataset.isfitoolAction]);
				$(window.FrameDocument, window.FrameWindow).bind('keydown', this.dataset.isfitoolShortcut, Isfitool.Gui[this.dataset.isfitoolAction]);
			}
		});
	},
	
	undo : function () {
		if (Isfitool.WysiwygEditor.isActive) 
		{
			Isfitool.WysiwygEditor.undo();
		} else
		{
			Isfitool.Undo.undo();
		}
		Isfitool.Builder.selectNode();
	},
	
	redo : function () {
		if (Isfitool.WysiwygEditor.isActive) 
		{
			Isfitool.WysiwygEditor.redo();
		} else
		{
			Isfitool.Undo.redo();
		}
	...
	save : function () {
		$('#textarea-modal textarea').val(Isfitool.Builder.getHtml());
	...
	saveAjax : function () {
		...
		var url = Isfitool.FileManager.getCurrentUrl();
		
		return Isfitool.Builder.saveAjax(url, null, function (data) {
			$('#message-modal').modal().find(".modal-body").html("File saved at: " + data);
		});		
	},
	
	download : function () {
		filename = /[^\/]+$/.exec(Isfitool.Builder.iframe.src)[0];
		uriContent = "data:application/octet-stream,"  + encodeURIComponent(Isfitool.Builder.getHtml());
	...
	toggleEditor : function () {
		$("#isfitool-builder").toggleClass("bottom-panel-expand");
		$("#toggleEditorJsExecute").toggle();
		Isfitool.CodeEditor.toggle();
	},
	
	toggleEditorJsExecute : function () {
		Isfitool.Builder.runJsOnSetHtml = this.checked;
	},
	
	preview : function () {
		(Isfitool.Builder.isPreview == true)?Isfitool.Builder.isPreview = false:Isfitool.Builder.isPreview = true;
		$("#iframe-layer").toggle();
		$("#isfitool-builder").toggleClass("preview");
	},
...
//Pages, file/components tree 
	newPage : function () {
		...
		newPageModal.modal("show").find("form").off("submit").submit(function( event ) {
			...
			Isfitool.FileManager.addPage(name, title, url);
			event.preventDefault();

			return Isfitool.Builder.saveAjax(url, startTemplateUrl, function (data) {
					Isfitool.FileManager.loadPage(name);
					Isfitool.FileManager.scrollBottom();
	...
	setDesignerMode : function () {
		...
		Isfitool.Builder.setDesignerMode(designerMode);
	},
//layout
	...
	toggleFileManager: function () {
		Isfitool.Gui.togglePanel("#filemanager", "--builder-filemanager-height");
	},
	
	toggleLeftColumn: function () {
		Isfitool.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
	},
	
	toggleRightColumn: function () {
		Isfitool.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
		var rightColumnEnabled = this.attributes["aria-pressed"].value == "true";

		$("#isfitool-builder").toggleClass("no-right-panel");
		$(".component-properties-tab").toggle();
		
		Isfitool.Components.componentPropertiesElement = (rightColumnEnabled ? "#right-panel" :"#left-panel") +" .component-properties";
		if ($("#properties").is(":visible")) $('.component-tab a').show().tab('show'); 

	},
}

Isfitool.StyleManager = {
...
Isfitool.FileManager = {
	...
	init: function() {
		...
		$(this.tree).on("click", "li[data-page] label", function (e) {
			...
			if (page) Isfitool.FileManager.loadPage(page);
			return false;			
		})
		...
		$(this.tree).on("click", "li[data-component] label ", function (e) {
			...
			Isfitool.Builder.frameHtml.animate({
				scrollTop: $(node).offset().top
			}, 1000);

			Isfitool.Builder.selectNode(node);
			Isfitool.Builder.loadNodeComponent(node);
	...
	addPage: function(name, data) {
		...
		if (data.folder)
		{
			if (!(folder = this.tree.find('li[data-folder="' + data.folder + '"]')).length) 
			{
				...
				folder = $(tmpl("isfitool-filemanager-folder", data));
		...
		folder.append(
			tmpl("isfitool-filemanager-page", data));
	},
	...
	addComponent: function(name, url, title, page) {
		$("[data-page='" + page + "'] > ol", this.tree).append(
			tmpl("isfitool-filemanager-component", {name:name, url:url, title:title}));
	},
				...
				if (node.hasChildNodes()) {
					for (var j = 0; j < node.childNodes.length; j++) {
						...
						if (child && child["attributes"] != undefined && 
							(matchChild = Isfitool.Components.matchNode(child))) 
						{
	...
	loadPage: function(name, allowedComponents = false, disableCache = true) {
		...
		Isfitool.Builder.loadUrl(url + (disableCache ? (url.indexOf('?') > -1?'&':'?') + Math.random():''), 
			function () { 
				Isfitool.FileManager.loadComponents(allowedComponents); 
			});
	},
